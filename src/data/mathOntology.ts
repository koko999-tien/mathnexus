...
export const MATH_ATOMS: MathAtom[] = [
  ...
  { id:'mechanics-state', conceptId:'classical-mechanics', kind:'definition', title:'Trạng thái cơ học', summary:'Trạng thái của một hạt được mô tả bằng vị trí và vận tốc tại một thời điểm.', formula:'$(\\mathbf r,\\mathbf v)$', tags:['state','position','velocity'] },
  { id:'newton-second-law', conceptId:'classical-mechanics', kind:'theorem', title:'Định luật II Newton', summary:'Gia tốc được xác định bởi tổng lực tác dụng chia cho khối lượng.', formula:'$m\\ddot{\\mathbf r}=\\sum \\mathbf F$', dependsOn:['mechanics-state'] },
  { id:'mechanics-momentum', conceptId:'classical-mechanics', kind:'definition', title:'Động lượng', summary:'Động lượng tuyến tính là tích giữa khối lượng và vận tốc.', formula:'$\\mathbf p=m\\mathbf v$', dependsOn:['mechanics-state'] },
  { id:'momentum-conservation', conceptId:'classical-mechanics', kind:'theorem', title:'Bảo toàn động lượng của hệ cô lập', summary:'Nếu tổng ngoại lực bằng 0 thì tổng động lượng của hệ không đổi theo thời gian.', formula:'$\\frac{d}{dt}\\sum_i \\mathbf p_i=0$', dependsOn:['mechanics-momentum','newton-second-law'] },
  { id:'mechanics-frame-misconception', conceptId:'classical-mechanics', kind:'misconception', title:'Mọi hệ quy chiếu đều dùng trực tiếp F=ma', summary:'Dạng Newton đơn giản áp dụng trực tiếp trong hệ quán tính; hệ phi quán tính cần lực quán tính bổ sung.', dependsOn:['newton-second-law'] },

  { id:'gravity-force-law', conceptId:'newtonian-gravity', kind:'definition', title:'Định luật hấp dẫn nghịch đảo bình phương', summary:'Hai khối lượng hút nhau theo phương nối tâm với độ lớn giảm theo bình phương khoảng cách.', formula:'$F=G\\frac{m_1m_2}{r^2}$' },
  { id:'gravity-vector-acceleration', conceptId:'newtonian-gravity', kind:'definition', title:'Gia tốc hấp dẫn dạng vector', summary:'Gia tốc của vật i là tổng đóng góp vector từ mọi vật j khác.', formula:'$\\ddot{\\mathbf r}_i=G\\sum_{j\\ne i}m_j\\frac{\\mathbf r_j-\\mathbf r_i}{\\|\\mathbf r_j-\\mathbf r_i\\|^3}$', dependsOn:['gravity-force-law'] },
  { id:'gravity-potential-energy', conceptId:'newtonian-gravity', kind:'definition', title:'Thế năng hấp dẫn', summary:'Mỗi cặp khối lượng đóng góp một thế năng âm tỷ lệ nghịch với khoảng cách.', formula:'$U_{ij}=-G\\frac{m_im_j}{r_{ij}}$', dependsOn:['gravity-force-law'] },
  { id:'gravity-two-body-example', conceptId:'newtonian-gravity', kind:'example', title:'Quỹ đạo gần tròn quanh khối lượng lớn', summary:'Với khối lượng trung tâm lớn, tốc độ quỹ đạo tròn xấp xỉ căn của GM/r.', formula:'$v\\approx\\sqrt{GM/r}$', dependsOn:['gravity-force-law'] },
  { id:'gravity-softening-misconception', conceptId:'newtonian-gravity', kind:'misconception', title:'Softening không phải lực đẩy vật lý', summary:'Tham số ε trong mô phỏng chỉ làm mềm singularity số ở khoảng cách rất nhỏ; nó không phải một định luật hấp dẫn mới.', dependsOn:['gravity-vector-acceleration'] },

  { id:'nbody-coupled-system', conceptId:'nbody-problem', kind:'definition', title:'Hệ ODE ghép 6N chiều', summary:'Mỗi vật có ba tọa độ vị trí và ba thành phần vận tốc; toàn hệ tiến hóa đồng thời qua một hệ phương trình vi phân ghép.', formula:'$\\dot{\\mathbf r}_i=\\mathbf v_i,\\quad \\dot{\\mathbf v}_i=\\mathbf a_i(\\mathbf r_1,\\dots,\\mathbf r_N)$', dependsOn:['gravity-vector-acceleration'] },
  { id:'nbody-no-general-closed-form', conceptId:'nbody-problem', kind:'misconception', title:'N-body không có một công thức nghiệm đóng chung', summary:'Từ ba vật trở lên, bài toán tổng quát không có biểu thức đóng đơn giản cho mọi điều kiện đầu; mô phỏng số trở thành công cụ trung tâm.', dependsOn:['nbody-coupled-system'] },
  { id:'nbody-step-size', conceptId:'nbody-problem', kind:'subskill', title:'Chọn bước thời gian', summary:'Bước quá lớn có thể làm sai pha quỹ đạo hoặc làm năng lượng trôi nhanh dù hình ảnh vẫn có vẻ hợp lý.', dependsOn:['nbody-coupled-system'] },
  { id:'nbody-energy-drift', conceptId:'nbody-problem', kind:'subskill', title:'Đọc energy drift', summary:'Theo dõi độ lệch năng lượng tương đối giúp đánh giá chất lượng tích phân số trong một hệ bảo toàn gần lý tưởng.', formula:'$\\delta_E=\\left|\\frac{E(t)-E(0)}{E(0)}\\right|$', dependsOn:['gravity-potential-energy','nbody-step-size'] },
  { id:'nbody-lab-application', conceptId:'nbody-problem', kind:'application', title:'Gravity Lab của MathNexus', summary:'Thay đổi seed, số vật thể, tốc độ và softening để quan sát quỹ đạo, hiệu năng và energy drift trên cùng một hệ.', body:'Simulation dùng đơn vị vô thứ nguyên và CPU Float64; đồ họa là trực quan hóa của trạng thái số chứ không phải dữ liệu thiên văn thực.', dependsOn:['nbody-energy-drift'], tags:['simulation','gravity lab'] },
  { id:'nbody-stability-exercise', conceptId:'nbody-problem', kind:'exercise', title:'Thí nghiệm độ ổn định số', summary:'Giữ nguyên seed, tăng tốc mô phỏng rồi so sánh energy drift; giải thích vì sao thay đổi bước hiệu dụng làm kết quả kém ổn định.', difficulty:'Vừa', dependsOn:['nbody-step-size','nbody-energy-drift'] },

  { id:'hamiltonian-phase-space', conceptId:'hamiltonian-mechanics', kind:'definition', title:'Không gian pha', summary:'Một trạng thái được biểu diễn bằng tọa độ q và động lượng p thay vì chỉ vị trí.', formula:'$(\\mathbf q,\\mathbf p)$' },
  { id:'hamiltonian-definition', conceptId:'hamiltonian-mechanics', kind:'definition', title:'Hamiltonian', summary:'Trong nhiều hệ cơ học, Hamiltonian biểu diễn tổng động năng và thế năng.', formula:'$H(\\mathbf q,\\mathbf p)=T+V$', dependsOn:['hamiltonian-phase-space'] },
  { id:'hamilton-equations', conceptId:'hamiltonian-mechanics', kind:'theorem', title:'Phương trình Hamilton', summary:'Động lực học được viết thành hai hệ phương trình bậc nhất đối xứng theo q và p.', formula:'$\\dot q_i=\\frac{\\partial H}{\\partial p_i},\\quad \\dot p_i=-\\frac{\\partial H}{\\partial q_i}$', dependsOn:['hamiltonian-definition'] },
  { id:'symplectic-integrator', conceptId:'hamiltonian-mechanics', kind:'application', title:'Integrator symplectic và leapfrog', summary:'Leapfrog bảo toàn cấu trúc pha tốt hơn Euler đơn giản trong nhiều hệ Hamilton, nên phù hợp cho quỹ đạo dài hạn.', dependsOn:['hamilton-equations','nbody-energy-drift'] },
  { id:'symplectic-energy-misconception', conceptId:'hamiltonian-mechanics', kind:'misconception', title:'Symplectic không có nghĩa năng lượng luôn chính xác tuyệt đối', summary:'Năng lượng vẫn dao động và sai số vẫn tồn tại; ưu điểm là hành vi dài hạn thường có cấu trúc tốt hơn.', dependsOn:['symplectic-integrator'] },
